...
const LoginForm = () => {
  ...
  useEffect(() => {
    if (currentUser) {
      const from = location.state?.from?.pathname || `/dashboard/${currentUser.role}`;
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (result.isTestAccount) {
        // Redirect to test account role selector
        ...
        <div className="bg-white py-8 px-10 shadow rounded-lg">
          ...
          <form className="space-y-6" onSubmit={handleLogin}>
            ...
            <div>
              <button
                type="submit"
                className={`w-full flex justify-center py-3 px-4 border border-transparent rounded-full shadow-sm text-sm font-medium text-white bg-${colorMap[role]}-600 hover:bg-${colorMap[role]}-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-${colorMap[role]}-500`}
              >
                Sign in
              </button>
            </div>
